...
bash scripts/11_build_dense_index.sh "$CORPUS_PATH" "$INDEX_PATH" "$META_PATH"
bash scripts/21_run_dense_retrieve.sh "$INDEX_PATH" "$META_PATH" "$QUERIES_PATH" "$RUN_DENSE"
bash scripts/40_rerank.sh "$RUN_DENSE" "$RUN_RERANK" "$QUERIES_PATH" "$META_PATH"
bash scripts/60_RAG.sh "$INDEX_PATH" "$META_PATH" "$QUERIES_PATH" "$OUTDIR/answers.jsonl"
